...
function main() {
	addPost(preRoute+"/perso/add/",(req,res)=>{
		...
		if (err.length != 0) {
			...
			BDD.perso.add(nom,pv,play,img,atks,function (data) {
    ...
    addRoute(preRoute+"/perso/get/:id",(req,res)=>{
        ...
        BDD.perso.get(id,function (data) {
			res.status(200).json(data);
		})
    });
    ...
    addPost(preRoute+"/perso/del/",(req,res)=>{
        ...
        BDD.perso.del(id,function (data) {
			res.redirect(preRoute+"/perso");
		})
    });
    ...
    addRoute(preRoute+'/perso', function(request, response) {
        response.sendFile(__dirname + pathHTML +'/perso.html');
    });}
