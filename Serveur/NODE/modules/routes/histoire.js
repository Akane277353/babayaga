...
function main() {
	addPost(preRoute+"/histoire/add/",(req,res)=>{
		...
		if (err.length != 0) {
			...
			BDD.histoire.add(description,chaos,combat,img,fond,choixs,function (data) {
    ...
    addRoute(preRoute+"/histoire/get/:id",(req,res)=>{
        ...
        BDD.histoire.get(id,function (data) {
			res.status(200).json(data);
		})
    });
    ...
    addPost(preRoute+"/histoire/del/",(req,res)=>{
        ...
        BDD.histoire.del(id,function (data) {
			res.redirect(preRoute+"/histoire");
		})
    });
    ...
    addPost(preRoute+"/histoire/mod/",(req,res)=>{
		...
		if (err.length != 0) {
			...
			BDD.histoire.mod(id,description,chaos,combat,img,fond,choixs,function (data) {
				res.redirect(preRoute+"/histoire");
			})
		}
    ...
    addRoute(preRoute+'/histoire', function(request, response) {
        response.sendFile(__dirname + pathHTML +'/histoire.html');
    });
	addRoute('/stat', function(request, response) {
        response.sendFile(__dirname + pathHTML +'/stat.html');
    });}
