...
function main() {
	addPost("/state/add",(req,res)=>{
		const raw = (req.body.choix);
		if (typeof raw != undefined) {
			let token = newToken();
			let choix = raw.split("!")
			for (var i = 0; i < choix.length-1; i++) {
				BDD.Rstate.add(token,i,choix[i],choix[i+1])
			}
    ...
    addRoute("/state/get/:token",(req,res)=>{
    	const token = (req.params.token);
        BDD.Rstate.getToken(token,function (data) {
			res.status(200).json(data);
		})
    });

    addRoute("/state/get/:chunk/:choix",(req,res)=>{
    	const chunk = (req.params.chunk);
    	const choix = (req.params.choix);

        BDD.Rstate.getChunk(chunk,choix,function (data) {
			res.status(200).json(data);
		})
    });
