Choix = class{
	...
	add(txt,suite,callback){
		let sql = "INSERT INTO `choix`(`id`, `suivant`,`txt`) VALUES ";
		sql = sql + "(null,'"+suite+"','"+txt+"');";
		let DB = this.db
		this.db.query(sql, function (err, result){
			if (err) throw err;
			let sql = "SELECT MAX(id) AS 'id' FROM choix;";
	...
	get(id,callback){
		let sql = "SELECT choix.* ,histoire.chaos FROM choix JOIN histoire ON histoire.id=choix.suivant WHERE choix.id="+id+" ORDER BY choix.id DESC;" ;
		this.db.query(sql, function (err, result) {
			if (err) throw err;
			callback(result)
		});
	...
	del(id,callback){
		let sql = "DELETE FROM `choix` WHERE `id`="+id+";" ;
		this.db.query(sql, function (err, result) {
			if (err) throw err;
			callback(result)
		});
	...
	purge(id,UPThis,callback){
		let sql = "SELECT * FROM choix WHERE suivant="+id+";";
		const This = this;
		this.db.query(sql, function (err, result){
			if (err) throw err;
			let sql = "DELETE FROM choix WHERE suivant="+id+";";
...
function format(ls,callback,choix,indice=0,res=[]) {
	...
	if (ls.length<=indice) {
		...
		CHOIX.get(el.choix1 ,function (res1) {
			...
			if (el.choix1 != "-1") {
				if (res1.length == 0) {
					console.log("	=>	histoire id : "+ls[indice]["id"]+" choix1 : "+el.choix1)
				}
				json.txt=res1[0].txt;
				json.id=res1[0].id;
			...
			CHOIX.get(el.choix2 ,function (res2) {
				...
				if (el.choix2 != "-1") {
					if (res2.length == 0) {
						console.log("	=>	histoire id : "+ls[indice]["id"]+" choix2 : "+el.choix2)
					}
					json.txt=res2[0].txt;
					json.id=res2[0].id;
				...
				CHOIX.get(el.choix3 ,function (res3) {
					...
					if (el.choix3 != "-1") {
						if (res3.length == 0) {
							console.log("	=>	histoire id : "+ls[indice]["id"]+" choix3 : "+el.choix3)
						}
						json.txt=res3[0].txt;
						json.id=res3[0].id;
	...
	class:function () {
		return class {
			...
			add(description,chaos,combat,img,fond,choix,callback,id=null){
				...
				let ids = [];
				if (choix.length == 0) {
					let sql = "INSERT INTO `histoire`(`id`, `description`,`combat`,`chaos`,`img`,`fond`,`choix1`,`choix2`,`choix3`) VALUES " ;
			...
			get(id,callback){
				let sql = "SELECT * FROM histoire WHERE id="+id+" ;" ;
				const CHOIX = this.choix;
				this.db.query(sql, function (err, result) {
					if (err) throw err;
					format(result,function (res) {
			...
			clearChoix(id){
				let sql = "UPDATE `histoire` SET choix1=-1 WHERE `choix1`="+id+";" ;
				this.db.query(sql, function (err, result) {
					if (err) throw err;
				});
				sql = "UPDATE `histoire` SET choix2=-1 WHERE `choix2`="+id+";" ;
				this.db.query(sql, function (err, result) {
					if (err) throw err;
				});
				sql = "UPDATE `histoire` SET choix3=-1 WHERE `choix3`="+id+";" ;
				this.db.query(sql, function (err, result) {
					if (err) throw err;
				});
			}
			...
			del(id,callback){
				...
				This.get(id,function (rawel) {
					...
					let sql = "DELETE FROM `histoire` WHERE `id`="+id+";" ;
					This.db.query(sql, function (err, result) {
						if (err) throw err;
						callback(result)
					});
